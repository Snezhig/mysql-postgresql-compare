...
class ProductFixturePrepare extends Command
{
    ...
    {
        $this->addArgument('icount', InputArgument::OPTIONAL, 'Items count', 10000);
        $this->addArgument('pcount', InputArgument::OPTIONAL, 'Props count', 2000);
    }


    protected function execute(InputInterface $input, OutputInterface $output)
    {
        $count = (int)$input->getArgument('icount');
        $props = $this->getProps((int)$input->getArgument('pcount'));
        $items = [];
        $part = 1;
        for ($i = 1; $i <= $count; $i++) {
            $items[] = [
